 ...
 */
public class GerarAvisoExtraControllerPT2 implements Initializable {
    ...
    private static int idSubOrc; //id do SubOrcamento
    ...
    private void btPreviousFired(ActionEvent event) throws Exception {
        GerarAvisoExtraPT2.getStage().close();
            ...
            if (day > 0 && day <= 31) {
                ...
                for (String s1 : fraccaoSel) {
                    for (String s : mesSel) {
                        ...
                        aviso.setDescricao("Aviso de Débito relativo a " + s + " do Orcamento: " + getOrcamentoInfo(idOrc).getAno() + " em relação ao débito da(s) seguinte(s) rubrica(s): " + getAllRubricas(idSubOrc) + ".");
                        ...
                        if (s.equals("Dezembro")) { // fazer o acerto das mensalidades em dezembro (pagar mais ou até menos conforme)
                            FracaoMensalidadesSubOrcamentoDAO fqoDAO = new FracaoMensalidadesSubOrcamentoDAO();
                            int mensalidade = fqoDAO.getMensalidade(s1, idSubOrc);
                            int valorAnual = fqoDAO.getValorAnual(s1, idSubOrc);
                            aviso.setValorPagar(valorAnual - (mensalidade * 11));
                            fqoDAO.close();
                        } else {
                            FracaoMensalidadesSubOrcamentoDAO fqoDAO = new FracaoMensalidadesSubOrcamentoDAO();
                            aviso.setValorPagar(fqoDAO.getMensalidade(s1, idSubOrc));
                            fqoDAO.close();
                        }
                        ...
                        aviso.setIdSubOrcamento(idSubOrc);
                        rDAO.close();

                        if (!dao.exist(aviso)) {
                            dao.adicionarAvisoDebito(aviso);
                            lvLog.getItems().add("Aviso de Débito relativo a " + s + " do Orcamento: " + getOrcamentoInfo(idOrc).getAno() + " da fração " + aviso.getCodFraccao() + " [OK - Aviso Gerado Com SUCESSO]");
                        } else {
                            lvLog.getItems().add("Aviso de Débito relativo a " + s + " do Orcamento: " + getOrcamentoInfo(idOrc).getAno() + " da fração " + aviso.getCodFraccao() + " [IGNORADO - Aviso já existente]");
                        }
    ...
    public void setIdOrc(int id) {
        GerarAvisoExtraControllerPT2.idOrc = id;
    }
    ...
    private void initFraccoesOrcamento() throws ClassNotFoundException, SQLException {
        FracaoSubOrcamentoDAO dao = new FracaoSubOrcamentoDAO();
        fraccoes = FXCollections.observableArrayList();

        for (FracaoSubOrcamento fr : dao.getAllFracoesSubOrcamento(idSubOrc)) {
            fraccoes.add(fr.getFracao());
        }
    ...
    void setIdSubOrc(int idSubOrcamentoSel) {
        GerarAvisoExtraControllerPT2.idSubOrc = idSubOrcamentoSel;
    }
